...
class KTX_Lib_NodeGroups(bpy.types.Operator):
    ...
    def execute(self, context):
     ...
     with bpy.data.libraries.load(filepath, link=False) as (data_from, data_to):
      data_to.node_groups = [name for name in data_from.node_groups if name.startswith(self.ngrs)]
     for ngr in data_to.node_groups:
      if ngr is not None:
       ...
       if ao:
        ...
        piet=ao.active_material.node_tree.nodes.new("ShaderNodeGroup")
    ...
    def draw(self,context):
     ...
     with bpy.data.libraries.load(filepath, link=True) as (data_from, data_to):
      for ngr in data_from.node_groups:
       layout.operator(KTX_Lib_NodeGroups.bl_idname, text=ngr,icon="NODETREE").ngrs = ngr
