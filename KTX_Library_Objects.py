...
class KTX_Lib_Objects(bpy.types.Operator):
    ...
    def execute(self, context):
     ...
     with bpy.data.libraries.load(filepath, link=False) as (data_from, data_to):
      data_to.objects = [name for name in data_from.objects if name.startswith(self.source)]
    ...
    def draw(self,context):
     ...
     with bpy.data.libraries.load(filepath, link=True) as (data_from, data_to):
      for ob in data_from.objects:
       layout.operator(KTX_Lib_Objects.bl_idname, text=ob,icon="MESH_ICOSPHERE").source = ob
